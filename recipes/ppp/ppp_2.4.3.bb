...
do_install_append () {
	...
	install -m 0755 ${WORKDIR}/linux-ppp ${D}${bindir}/
	install -m 0755 ${WORKDIR}/win-ppp ${D}${bindir}/
	install -m 0755 ${WORKDIR}/config-ppp ${D}${sbindir}/
	install -m 0644 ${WORKDIR}/options ${D}${sysconfdir}/ppp/
	install -m 0644 ${WORKDIR}/options ${D}${sysconfdir}/ppp/options.default
	install -m 0644 ${WORKDIR}/options.ttyS1 ${D}${sysconfdir}/ppp/
	install -m 0644 ${WORKDIR}/options.ttyS1 ${D}${sysconfdir}/ppp/options.ttyS1.default
	install -m 0644 ${WORKDIR}/winclient.chat ${D}${sysconfdir}/ppp/
	rm -rf ${D}/${mandir}/man8/man8
}

CONFFILES_${PN} = "${sysconfdir}/ppp/pap-secrets ${sysconfdir}/ppp/chap-secrets ${sysconfdir}/ppp/options ${sysconfdir}/ppp/options.ttyS1"
PACKAGES += "ppp-oa ppp-oe ppp-radius ppp-winbind ppp-minconn ppp-password ppp-tools"
FILES_${PN}        = "/etc /usr/bin /usr/sbin/chat /usr/sbin/pppd /usr/sbin/config-ppp"
